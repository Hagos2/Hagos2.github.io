...
//2
function getEmployeeNamesObjectsArray(department) {//call function,name object array
    if (Array.isArray(department))
        return department.map((item)=> {return {name:item.name}})
    let names = [];//variable
    for (let subdep of Object.values(department)) {
        let subDepNames = getEmployeeNamesObjectsArray(subdep);//recall function
...
//3
function getEmployeeNameSalary(department){//jhon:200000
    if (Array.isArray(department)) 
